...
const streamingRoleName = "STREAMERS"; // Replace with your streaming role name
...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  const hasStreamingRole = member.roles.cache.some(
    (role) => role.name === streamingRoleName
  );
  if (hasStreamingRole) {
    ...
    if (!isStreamingBefore && isStreamingNow) {
      ...
      streamingData.set(member.id, {
        startTime: start,
        displayName: member.displayName,
      }); // Store start time
      incrementStreamingCount(member.id, "start"); // Increment start count
      member.send(
        `Dear Streamer, \nDo not close your stream within 15 min. Thats not counting in your stream time. \nThanks.`
      );
      ...
      if (streamData) {
        ...
        const targetTextChannel = client.channels.cache.get(
          process.env.Text_Channel
        );
        const userId = member.id; // Replace with the actual user ID
        const counts = getStreamingCounts(userId);

        if (targetTextChannel && duration >= 900000) {
          const pkrtime = moment().tz("Asia/Karachi").format("LT");
          targetTextChannel.send(
            `=======================\n**Name:** ${displayName} \n**Streamed Time:** ${formattedDuration} \n**Current time:** ${moment().format(
              "MMMM Do YYYY"
            )} \n**Start Streamed Time:** ${moment(startTime).tz("Asia/Karachi").format("LT")} \n**End Streamed Time:** ${pkrtime} \nStreaming **${
              counts.start
            } times** and stopped streaming **${
              counts.stop + 1
            } times.**\n=======================\n\n`
          );
        }
